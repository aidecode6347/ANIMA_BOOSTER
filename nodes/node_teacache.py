...
class SdxlTeaCache:
    """
    Adaptive TeaCache for SDXL models.

    Speeds up inference by skipping redundant UNet computations
    between similar denoising steps.

    Specifically tuned for SDXL UNet architecture (safer default thresholds).
    """
    ...
    CATEGORY = "BSS/AnimaBooster"
    DESCRIPTION = (
        "Адаптивный TeaCache для SDXL: значительно ускоряет генерацию моделей SDXL, пропуская избыточные "
        "вычисления между близкими шагами сэмплинга. Настройки оптимизированы специально под архитектуру UNet."
    )
